(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('businessHoursCalculatorApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-businessHoursCalculatorApp-error');
                var entityKey = httpResponse.headers('X-businessHoursCalculatorApp-params');
                if (errorHeader) {
